...
記事の「M5三本からM15一本へ残るもの・戻らないもの」の模式図は、実レートを使わず、三本の人工M5を上記と同じ集約関数へ通して生成します。

```bash
python chapters/season2/ch02_minute_data_db/figures.py
```

既定の出力先は`results/reference/ch02_minute_data_db/figures/m5_to_m15_information_loss.png`です。
...
固定manifestの`code_commit`は、manifest生成時の作業workspaceが返した値を履歴として保持しています。末尾の`-dirty`も含めて生成時の事実であり、公開commit IDへの置き換えはしません。公開したコードの版は、このmanifestを含むGitHub commitで固定します。
